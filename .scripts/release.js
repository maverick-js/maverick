...
const packages = packagesDir.filter((p) => {
  return (
    !p.startsWith('.') && fs.existsSync(path.resolve(__dirname, '../packages', p, 'package.json'))
  );
});
const preId =
...
async function main() {
  ...
  if (stdout) {
    ...
    await runIfNotDry('git', ['commit', '--no-verify', '-m', `chore(release): v${targetVersion}`]);
  } else {
