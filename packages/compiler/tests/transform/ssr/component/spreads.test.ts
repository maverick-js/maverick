...
test('spread', () => {
  expect(ssr('<Foo {...a}  />')).toMatchInlineSnapshot(`
    ...
    let $_spread_1 = a;
    $$_create_component(Foo, $_spread_1, null, $_spread_1);
    "
  `);
});
...
test('multiple', () => {
  expect(ssr('<Foo {...a} {...b} {...{a: 1, b: 2}} />')).toMatchInlineSnapshot(`
    "import { $$_merge_props, $$_create_component } from "@maverick-js/ssr";
...
test('with attributes', () => {
  expect(
    ...
    $$_create_component(Foo, $_spread_1, null, $$_merge_props($_spread_1, {
        class: "...",
        "$class:foo": isFoo,
        "$var:foo": fooVar
    }));
